...
#' @param NULLObjPrefix Prefix for the fitted null model generated by Step1.
#'   The function reads `<NULLObjPrefix>.rda`.
#' @param SummaryPrefix Base prefix for Step2 summary result files. The
#'   function normalizes a trailing underscore when present, then writes one file per feature as
#'   `<SummaryPrefix>_chr<chrom>_<feature>.txt` when `chrom` is set, or
#'   `<SummaryPrefix>_allchr_<feature>.txt` when `chrom` is `NULL`.
...
getSummary <- function(genoFile,
  ...
  if (missing(SummaryPrefix) || !nzchar(SummaryPrefix)) {
    stop("'SummaryPrefix' must be provided.")
  }
  SummaryPrefix <- sub("_+$", "", SummaryPrefix)
