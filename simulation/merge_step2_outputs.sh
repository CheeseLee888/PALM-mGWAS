...
if [[ "${#STUDY_LIST[@]}" -eq 0 ]]; then
  echo "merge_step2_outputs(step2.2): no studies resolved from STUDIES/STUDY" >&2
  exit 1
fi

echo "merge_step2_outputs(step2.2): simulation root = ${SIMU_ROOT}"
echo "merge_step2_outputs(step2.2): studies = ${STUDIES_CSV}"
...
for study_name in "${STUDY_LIST[@]}"; do
  ...
  if [[ ! -d "${OUTPUT_DIR}" ]]; then
    echo "merge_step2_outputs(step2.2): missing output directory ${OUTPUT_DIR}" >&2
    exit 1
  fi
...
echo "merge_step2_outputs(step2.2): wrote ${STUDY_FILE}"
